...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    zeroyt7.on('open', () => {
        success('2', 'SUKSES TERSAMBUNG SILAHKAN GUNAKAN BOT.. ')
    })
...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `*Welcome @${num.split('@')[0]}*\n :  ${pushname}\n : _@${sender.split('@')[0]}\n: ${Metadata.participants} Member (s)\n : ${time2} Indonesian tim`
        ...
        but = [
          { buttonId: '#infogc', buttonText: { displayText: 'WELCOME ' }, type: 1 }
        ]
        sendButImage(mdata.id, wel, "Â© OCHOBOTZ", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        but = [
          { buttonId: '#infogc', buttonText: { displayText: 'GoodBey' }, type: 1 }
        ]
        sendButImage(mdata.id, out, "Â© OCHOBOTZ", buff, but)
      }
